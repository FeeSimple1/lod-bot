...
def add_resource(state, faction: str, amount: int) -> None:
    """
    Grant or spend Resources for *faction* (positive / negative amount).
    Automatically clamps and logs the change.
    """
    state["resources"][faction] += amount
    clamp_resources(state)
    push_history(state, f"{faction} Resources {'+' if amount >= 0 else ''}{amount}")
...
def shift_support(state, space_id: str, delta: int) -> None:
    ...
    """
    cur = state["support"][space_id]
    new = max(MIN_SUPPORT, min(MAX_SUPPORT, cur + delta))
    if new != cur:
        state["support"][space_id] = new
        push_history(state, f"Support shift in {space_id}: {cur:+d} → {new:+d}")
