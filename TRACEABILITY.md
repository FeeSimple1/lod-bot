...
| 8.3.5 | Events: Who/What/Where | PARTIAL (T7, P2) | bots/free_op_planner.py, bots/random_spaces.py, cards/effects/early_war.py, cards/effects/late_war.py, cards/effects/shared.py | tests/test_bot_free_ops.py, tests/test_early_war_cards.py, tests/test_event_space_selection.py | Shift routing → 8.3.6 OK (Session 21). Free-Command choices → faction priorities OK (free_op_planner + engine._plan_bot_free_op; card 84 Session 21). Maximise-Forts-then-pieces used for card 6. Q22 space-tie sweep CLOSED deck-wide (Session 68): every card-effect handler now resolves space ties via `random_spaces.pick_by_priority`/`pick_random_spaces` (the Random Spaces table), no rng embedded in any sort key.  **T7 who-ordering CLOSED (Session 70):** general `util/target_order.py` selector implements benefit order (executing → other friendly → random enemy Non-player first) and harm order (random enemy player first), faction ties by seeded roll (not the space table).  Routed through the clear faction-choice handlers — cards 18/44 (ineligibility harm), 80 (remove-pieces harm; replaced a non-§8.3.5 'most pieces' heuristic), 66 (French-or-Patriots benefit) — and base_bot force_if_eligible_enemy unified onto it (card 67 was already compliant).  **Per-card who-choice audit DONE (Session 73):** every handler's faction-valued decision classified (7 already routed; 3 hand-rolled-conforming — card 32 executor-benefit, card 70 Event-Instruction-driven, card 52 executor-battler with the card-named FRENCH fallback for a battle-incapable INDIANS executor; 10 what-choice/coalition-only) and the two non-conforming defaults fixed onto the T7 helper: card 67 (Royalist executor hardwired FRENCH instead of the seeded random-enemy pick its sibling 66 uses) and card 87 (fixed executor-blind removal tag order in Pennsylvania removed friendly Indian War Parties first for a Royalist executor; now §8.3.5 harm order across factions, units-before-bases within). Also swept the last 15 pre-Q22 tie sites out of the four bot files (rng-in-sort-key / dict-order picks in the card-80 presets, British garrison phase-2b targets, displacement city+province, all four leader redeploys, Cornplanter village target, loyalist desertion) onto pick_by_priority/pick_random_spaces; the British card-80 preset's non-sheet most-cities faction heuristic now uses §8.3.5 harm order.  beneficiary_order hardened against unknown executors (bare test states).  Canary rebaselined; goldens hold.  **Flowchart-question-spaces-first IMPLEMENTED (Session 73):** a space-conditioned "Event or Command?" bullet now records its FULL matching set in state["_event_q_spaces"] (B2 bullets 3a/3b/3c: Tories->Active-Opposition-no-Tories, Fort->Colony-without, Regulars->City/Colony within the card's placement domain; P2 bullet 2: Underground Militia->Active-Support/Village-with-none; F2/I2 have no space-conditioned questions — their bullets test aggregates or piece pools).  random_spaces.pick_by_priority ranks question-matching spaces above ALL substantive keys and pick_random_spaces exhausts them first (the Manual places the bullet above the piece/shift priorities); base_bot._execute_event clears the key right after the handler so free-Command planning (§8.3.5's own-priorities bullet) and later turns never see it, and _choose_event_vs_flowchart clears stale sets before each bullet list.  Tests: test_event_question_spaces.py (picker units, B2/P2 integrations, lifecycle).  T7 residuals now CLOSED. |
...
- **T7 (P2) who-ordering IMPLEMENTED (Session 70).** §8.3.5 benefit/harm
  ...
  §8.2 space-tie half closed deck-wide in Session 68 (Q22).  RESIDUALS
  CLOSED (Session 73): per-card who-choice audit done (see the 8.3.5 row)
  and the flowchart-question-spaces-first bullet implemented
  (test_event_question_spaces.py).  Tests: test_t7_target_ordering.py.
